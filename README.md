...
接管系统桌面图标的显示:隐藏原来的 `SysListView32` 桌面图标层,改用 **四块半透明、圆角、带柔和阴影的面板** 重新呈现桌面内容,并支持在面板之间自由拖拽图标。
...
- **拖动面板(位置)**:按住标题栏拖动 —— 采用**自实现移动**(不依赖系统 HTCAPTION 模态循环,与 `WS_EX_NOACTIVATE`、定时器 z 序维护互不干涉,右侧此前"拖不动"的问题由此根治);增量按**屏幕坐标**求差(客户区坐标会随窗口移动换帧,混帧会导致抖动/回弹);点击标题会把该面板提到本组最上层(防被相邻面板遮挡)。位置持久化。
